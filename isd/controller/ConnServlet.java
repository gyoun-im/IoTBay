...
public class ConnServlet extends HttpServlet {
    ...
    private PaymentDBManager paymentDBManager;              //Share database manager
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            paymentDBManager = new PaymentDBManager(conn);  //Creates an instance of AccessDBManager (from model.dao) and adds it to current session
        } catch (SQLException ex) {
            Logger.getLogger(ConnServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
        //export the DB manager to the view-session (JSPs)
        session.setAttribute("manager", paymentDBManager);     //Add manager to the session
    }
